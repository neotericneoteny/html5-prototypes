...
jasmine.JQuery.elementToString = function(element) {
	...
	if (domEl == undefined || domEl.cloneNode) return $('<div />').append($(element).clone()).html()
	...
	var bindMatcher = function(methodName) {
		...
		jasmine.JQuery.matchersClass[methodName] = function() {
			if (this.actual && (this.actual instanceof $ || jasmine.isDomNode(this.actual))) {
				...
				var element
				if (this.actual.get && (element = this.actual.get()[0]) && !$.isWindow(element) && element.tagName !== "HTML") this.actual = jasmine.JQuery.elementToString(this.actual)
				return result
			}
...
beforeEach(function() {
	...
	this.addMatchers({
		toHaveBeenTriggered: function() {
			...
			this.message = function() {
				return ["Expected event " + eventName + " to have been triggered on " + selector, "Expected event " + eventName + " not to have been triggered on " + selector]
			}
	...
	this.addMatchers({
		toHaveBeenPreventedOn: function(selector) {
			this.message = function() {
				return ["Expected event " + this.actual + " to have been prevented on " + selector, "Expected event " + this.actual + " not to have been prevented on " + selector]
			}
